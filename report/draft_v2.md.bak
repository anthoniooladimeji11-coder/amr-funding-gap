...
This work asks: **for the leading AMR pathogens, how closely does R&D funding share track attributable burden share, where is the gap compounded by a thin late-stage pipeline, and how is the misalignment likely to evolve to 2030?** We answer with a Global Misalignment Index (GMI), a pipeline cross-flag that distinguishes neglected pathogens from those served despite low funding, and a Bayesian projection of XDR prevalence that propagates uncertainty into a projected 2030 GMI.

## 2. Methods

We assembled and verified seven analytical layers (full code at the linked repository; complete decision log preserved as `DECISIONS.md`).
...
**[FIGURE 1.** Burden share (x-axis) vs funding share (y-axis), per pathogen; bubble size encodes late-stage pipeline share; bubble colour encodes quadrant. Diagonal line shows perfect alignment. Pathogens above the diagonal are overfunded; below are underfunded. *A. baumannii* and *K. pneumoniae* sit in the bottom-right underfunded zone, with *A. baumannii* visibly the smallest bubble (thin pipeline). Generated by `src/amr_gap/figure1.py`; rendered at `data/processed/figures/fig1_quadrant.png`.]**

## 3b. Projected 2030 misalignment

We fit the Beta-binomial random-walk model separately to *K. pneumoniae* and *A. baumannii* XDR prevalence series. Markov-chain Monte Carlo diagnostics were satisfactory for both fits: rank-normalised split R-hat was within 1.01 of unity for all parameters, effective sample size was satisfactory, and the *Klebsiella* fit produced 6 of 8,000 divergent transitions (0.07%, below the 0.5% threshold of concern). Posterior random-walk step SD (sigma on the logit scale) was 0.30 for *A. baumannii* and 0.17 for *K. pneumoniae*.

*Acinetobacter baumannii* XDR has saturated at approximately 63–65% since 2018, and the model correctly reflects this plateau: the 2030 posterior mean is 61.5% with an 80% credible interval of 39.8–81.4%, conveying that the central estimate stays near the current plateau while six years of accumulated uncertainty allow for substantial movement in either direction. *Klebsiella pneumoniae* XDR rose from 6.5% in 2012 to 18.6% in 2024, and the 2030 posterior mean is 19.4% (80% UI 11.9–27.7%), reflecting modest continued upward drift consistent with recent flatter years. The model is conservative by design: a random walk forecasts continued movement at recent rates and cannot extrapolate a trend, which we report as a feature against over-extrapolation rather than a limitation.

We translate projected prevalences into projected burdens under the assumption that each pathogen's attributable burden scales linearly with its XDR prevalence relative to 2019 (D-019). The four unprojected pathogens hold their 2019 burdens; funding shares are held constant. Propagating uncertainty through the index via per-draw recomputation yields a **projected 2030 GMI of 27.5% (80% credible interval 23.0–32.0%, 95% CI 20.7–35.1%)**, a modest widening from the current 25.3%. The widening is driven by *K. pneumoniae*, whose projected misalignment grows from –12.4 to –17.3 pp (80% UI –25.5 to –9.3); *A. baumannii* shows a small relative narrowing as *K. pneumoniae*'s growing burden share absorbs proportional weight in the index (–6.1 to –4.5 pp). The 80% UI of the projected GMI overlaps the current value, meaning we report the central estimate drifts upward but cannot claim with high confidence the index will worsen.

## 4. Discussion and policy implications

The doubly-neglected finding is the central message: **two of the six leading AMR pathogens (*A. baumannii* and *K. pneumoniae*) are receiving disproportionately little funding relative to the deaths they cause, and that gap is compounded by a thin late-stage pipeline.** These two pathogens are precisely the WHO Priority 1 critical-priority carbapenem-resistant Gram-negatives. The convergence of three independent signals (burden share, funding share, pipeline share) on the same conclusion strengthens it materially. The Bayesian projection adds a fourth: the gap for *K. pneumoniae* widens further under current trends.

**Implications for R&D allocation.** A reallocation on the order of 10 to 15 percentage points of within-the-six funding share toward *A. baumannii* and *K. pneumoniae* would close the misalignment for these pathogens. In absolute terms, applied to the USD 4.17 billion of pathogen-named funding in our base FAP, that is roughly USD 400 to 600 million of cumulative R&D effort. Even the most generous redistribution assumption (the equal broad-pool sensitivity, which credits broad Gram-negative funding fully to specific Gram-negative pathogens) leaves a GMI of 15.7%, so the qualitative case for reallocation does not depend on a particular attribution rule. The projection suggests the case strengthens, not weakens, on a 2030 horizon.
...
**Future extensions.** A hierarchical projection model partial-pooling across all six pathogens would yield a fuller projected GMI but requires additional LCA phenotyping infrastructure for the remaining four pathogens. Regional disaggregation requires regional funding attribution, which the current Hub data does not robustly support. Drug-class resolution requires extending the Hub taxonomy or linking to a separate pipeline-tracking dataset such as the WHO antibacterial agents preclinical and clinical pipelines.
...
Stan Development Team. Stan Modeling Language Users Guide and Reference Manual. (2024). mc-stan.org
